...
function endNight(room) {
    ...
    if (killedId && killedId !== savedId) {
        ...
        io.to(room).emit('newMessage', { sender: "النظام", text: `🏥 حاول القاتل تنفيذ جريمته لكن الطبيب كان في المرصاد وأنقذ الضحية! ✅` });
    } else {
        io.to(room).emit('newMessage', { sender: "النظام", text: `🌅 مر الليل بسلام ولم يمت أحد.` });
    }
...
function endDay(room) {
    ...
    if (victimId) {
        ...
        if (victim) { victim.isAlive = false; victim.isSpectator = true; io.to(room).emit('newMessage', { sender: "النظام", text: `⚖️ قرر الشعب إعدام ${victim.name}.` }); }
    }
...
function checkGameOver(room) {
    ...
    if (mafia === 0 && rd.phase !== "waiting") { io.to(room).emit('newMessage', { sender: "النظام", text: "🏆 فوز السكّان! تم القضاء على المافيا." }); return true; }
    if (mafia >= citizens && rd.phase !== "waiting") { io.to(room).emit('newMessage', { sender: "النظام", text: "😈 فوز المافيا! لقد سيطروا على المدينة." }); return true; }
    return false;
}
